/*
    ...
    Modules:
        INT_WXAY_vector: Parameterized design for WXAYB design
        INT_mul_vector: Parameterized design for INT multiplier
        ADDER_TREE_{B}: Adder tree module for different block size
*/

`include "params.vh"

module INT_WXAY_vector (
    input  wire                          clk,
...
/*
    INT multipliers and convert to fixed point, WXAY
*/

// Integer vector multiply: B lane-wise multiplies
module INT_mul_vector (
    input  wire [X*B-1:0] WEIGHT,
    input  wire [Y*B-1:0] ACT,
    ...
    generate
        for (g = 0; g < B; g = g + 1) begin : ELEM
            // Slice out each lane
            wire signed [X-1:0] w_s = WEIGHT[X*g +: X];
            wire signed [Y-1:0] a_s = ACT   [Y*g +: Y];

            // Products (full precision X+Y)
            wire signed [PRODUCT_WIDTH-1:0] p_sgn = $signed(a_s) * $signed(w_s);

            // Pack into output vector
            assign PRODUCT[g*PRODUCT_WIDTH +: PRODUCT_WIDTH] = p_sgn;
        end
    endgenerate
